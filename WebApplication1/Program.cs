...
app.MapDelete("api/Admission/{VenorCode:int}", [Authorize] async (int venorcode, ModelDB db) =>
{
    Admission? admission = await db.Admissions.Where(u => u.VenorCode == venorcode).FirstOrDefaultAsync();
    if (admission == null) return Results.NotFound(new { message = "Поставка не найдена" });
    db.Admissions.Remove(admission);
    await db.SaveChangesAsync();
    return Results.Json(admission);
});
app.MapDelete("api/Sell/{Id:int}", [Authorize] async (int Id, ModelDB db) =>
{
    Sell? sell = await db.SellOrders.Where(u => u.Id == Id).FirstOrDefaultAsync();
...
{
    Admission? a = await db.Admissions.Where(u => u.Id == admission.Id).FirstOrDefaultAsync();
    ...
    db.Admissions.Update(a);
    await db.SaveChangesAsync();
    return Results.Json(a);
});
app.MapPut("api/Sell", [Authorize] async (Sell sell, ModelDB db) =>
{
    Sell? s = await db.SellOrders.Where(u => u.Id == sell.Id).FirstOrDefaultAsync();
